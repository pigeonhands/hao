...
use crate::{io::{DataReader, ReadData}, error::{Result, HaoError}, dotnet::{module::MaybeUninitEntries, md::streams::tables_stream::coded_tokens::{CodedToken, CodedTokenTarget}}};
...
impl<'a> BlobStream<'a> {
    pub fn from_reader(reader: DataReader<'a>) -> Result<Self> {
        Ok(Self { reader, reader_heap_offset: 0 })
    }
    ...
    pub fn get_signature_reader(&self, offset: u32, entries: &'a MaybeUninitEntries) -> Result<SignatureReader> {
        let offset = offset as usize;
        let mut new_reader = self.split_from(offset)?;
        let len = new_reader.read_compressed_u32().map_err(|_| {
            HaoError::InvalidStreamIndex("#Blob", self.heap_position())
        })? as usize;

        if len > new_reader.reader.remaning() {
            return Err(HaoError::InvalidStreamIndex("#Blob", self.reader_heap_offset + offset));
        }
        
        new_reader.slice(0..len).map(|r| SignatureReader::new(r, entries))
    }
    ...
    pub fn read_compressed_u32(&mut self) -> Result<u32> {
        ...
        read_compressed(self).map_err(|_| {
            HaoError::InvalidStreamIndex("#Blob", last_pos)
        })
    }
